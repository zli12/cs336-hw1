...
| 4 | Speedrun mods | Implement ReLU² FFN, value embeddings/skip connection, WSD schedule, ctx=1024 path; cumulative greedy ablation matrix on OWT subset starting from Phase-3 winner. | **DONE** (KEEP: WSD + ctx=1024; DROP: ReLU² + value embeds; val@2000=3.9365; cumulative -0.738 vs Phase 0.5) |
| 4.5 | Final LR sanity A/B | At ±sqrt(2) around the Phase-4 winner LR for 1500 steps on OWT subset. If shifted, adopt the better one for the final run. | **DONE** (winner lr=1.414e-3; val@2000=3.9209; cumulative -0.754 vs Phase 0.5) |
| 5 | Final run | ~3.25h A100-40GB run (compute-equivalent to 1.5h H100) with the cumulative-best config; write checkpoint + csv + console log; check wallclock ≤ 11700s. | **DONE** (45,000 steps in 10,584s; val=3.1904; cumulative -1.484 nats vs Phase 0.5; -0.83 nats vs 7.4 OWT 4.02 baseline) |
| 6 | Writeup | Generate text samples, plot wallclock-axis learning curve, write `7.5_leaderboard_report.md` with final val loss, ablation table, hardware note, samples. | **DONE** ([`7.5_leaderboard_report.md`](./7.5_leaderboard_report.md), [`7.5_leaderboard_learning_curve.svg`](./7.5_leaderboard_learning_curve.svg), [`experiments/generations/leaderboard/`](../generations/leaderboard/)) |
...
### Phase 4 — speedrun ablation matrix (each ~8 min OWT, 2000 steps) — **DONE**
...
- **Result (full table in [`7.1_experiment_log.md`](./7.1_experiment_log.md) §7.5 Phase 4):**
  - **WSD schedule (warmup 100, decay 20%): KEEP.** val@2000 4.1160 → **3.9951** (−0.121 nats vs Muon-fixed); the LR landing at 0 sharpens the snapshot enormously.
  - **ReLU² FFN: DROP.** val@2000 3.9951 → 4.0149 (+0.020 nats; ~13% faster but worse quality at this scale).
  - **Value embed @ L4: DROP.** val@2000 essentially identical (3.9952; within ±0.005 nats noise envelope). The U-net skip didn't move the needle at d=1024 L=8 / 2000 steps.
  - **ctx=1024 with bs=24 (same tokens/step as bs=96 ctx=256): KEEP.** val@2000 → **3.9365** (−0.059 nats); longer context wins given SDPA's O(T) memory.
  - **Phase 4 winner: WSD + ctx=1024 + bs=24 + Muon-mixed + full Tier-1 stack at d=1024 L=8.** val@2000 = **3.9365**.
  - Cumulative gain since Phase 0.5 baseline: **4.6744 → 3.9365 = −0.738 nats**.
  - Final-run wallclock budget at this config: 11700s × 107K tok/s / 24,576 tok/step ≈ **51K steps**, squarely inside the plan's 30–60K target band — exactly what's needed for cosine/WSD to actually decay.
- Drivers: [`scripts/run_phase4_speedrun_mods.sh`](../../scripts/run_phase4_speedrun_mods.sh) (covers 4a/4b) + ad-hoc single-config runs for 4c/4d at the same shape.

### Phase 4.5 — final LR sanity A/B (~30 min) — **DONE**
...
- **Result (full table in [`7.1_experiment_log.md`](./7.1_experiment_log.md) §7.5 Phase 4.5):**
  - val@2000 — 0.707e-3: 3.9636, 1.0e-3 (anchor): 3.9365, **1.414e-3: 3.9209 (winner)**.
  - Optimum drifts up by sqrt(2) when stacked with the longer-context + WSD-schedule combination.
  - **Phase 4.5 winner: AdamW lr=1.414e-3** (Muon muon-lr=0.02 unchanged).
  - Cumulative since Phase 0.5 baseline: 4.6744 → **3.9209 = −0.754 nats** at val@2000.
...
### Phase 5 — final compute-budgeted A100 run (~3.25h ≈ 11700s) — **DONE**
- Setup: cumulative best from Phase 4.5 (d=1024 L=8 num_heads=16 d_ff=2752 ctx=1024 bs=24 / Muon-mixed muon-lr=0.02 + AdamW lr=1.414e-3 / WSD warmup=100 decay_frac=0.2 / full Tier-1 stack / bf16 + SDPA + compile=default).
- Step budget tuned to actual observed throughput: `--max-steps 45000` so WSD's linear decay reaches LR=0 exactly at step 45,000 and the run finishes naturally inside the wallclock cap (--max-wallclock-sec 11400 added as a safety guard via the new flag in [`scripts/train_lm.py`](../../scripts/train_lm.py); see also `test_main_wallclock_guard_triggers_early_val_and_checkpoint` in `tests/test_train_lm_script.py`).
- **Result (full table in [`7.1_experiment_log.md`](./7.1_experiment_log.md) §7.5 Phase 5):**
  - **Final val_loss = 3.1904** at step 45,000 / 10,583.62s wallclock (= 2h 56min 23s).
  - Throughput: 106.2K tok/s training-only steady state; 1.106B tokens trained.
  - Cumulative gain since Phase 0.5 baseline (4.6744): **−1.484 nats** at val@45,000.
  - vs the 7.4 OWT main experiment (4.02 at 10K cosine steps, 2.26h on A10G): **−0.83 nats** at the compute-equivalent envelope.
  - WSD decay-phase contribution: 3.3975 (step 36,000) → 3.1904 (step 45,000) = **−0.207 nats** from the late-decay snapshot alone.
  - Compute envelope: 10,583.62s on A100-40GB / 2.17x conversion midpoint = **1.354h H100-equivalent**, well inside the 1.5h leaderboard envelope.
- Outputs: [`experiments/logs/leaderboard-final.csv`](../logs/leaderboard-final.csv), [`experiments/logs/leaderboard-final.console.log`](../logs/leaderboard-final.console.log), `experiments/checkpoints/leaderboard-final.pt`.
- Driver: [`scripts/run_phase5_final.sh`](../../scripts/run_phase5_final.sh).

### Phase 6 — deliverables (~30 min) — **DONE**
- Wallclock-x-axis val-loss curve via [`scripts/plot_runs.py`](../../scripts/plot_runs.py): [`7.5_leaderboard_learning_curve.svg`](./7.5_leaderboard_learning_curve.svg). Run end at wallclock 10,584 s on A100 (= 1.354 h on H100 at the canonical 2.17× midpoint), inside the 1.5 h H100 envelope.
- Generation samples via [`scripts/run_phase5_samples.sh`](../../scripts/run_phase5_samples.sh) (which calls [`scripts/generate_lm.py`](../../scripts/generate_lm.py) — that script needed `--qk-norm` / `--tie-embeddings` / `--logit-soft-cap` / `--use-sdpa` / `--value-embed-layers` flags added so the final-run checkpoint loads correctly). Five samples in [`experiments/generations/leaderboard/`](../generations/leaderboard/) mirroring the 7.4 OWT layout.
- Writeup [`7.5_leaderboard_report.md`](./7.5_leaderboard_report.md) with: final val loss `3.1904`, cumulative ablation table, learning-curve figure, samples, and explicit A100→H100 conversion (2.0–2.5× bf16-dense; final wallclock fits inside 1.5 h H100 under all three points of the ratio).
