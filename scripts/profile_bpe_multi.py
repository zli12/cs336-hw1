...
def main() -> None:
    parser = argparse.ArgumentParser(description="Profile multiprocessing BPE training with cProfile.")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument("--num-processes", type=int, default=8, help="Worker process count.")
    parser.add_argument(
        "--stats-out",
        type=Path,
        default=Path("data/profile-bpe-multi.pstats"),
        help="Binary cProfile stats output path.",
    )
    parser.add_argument(
        "--report-out",
        type=Path,
        default=Path("data/profile-bpe-multi.txt"),
